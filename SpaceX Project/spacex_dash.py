...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                # Showing the Success vs. Failed counts for the site if a specific launch site was selected
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        count_df  = filtered_df['class'].value_counts().to_frame()
        fig = px.pie(count_df,  values='class',
        names= count_df.index,
        title='Successful Launches for Launch Site')
        return fig
...
def get_scatter_chart(entered_site, entered_payload):
    filter1 = spacex_df.loc[(spacex_df["Payload Mass (kg)"] >= entered_payload[0]) & (spacex_df["Payload Mass (kg)"] <= entered_payload[1])]
    filtered_df = filter1.loc[filter1["Launch Site"] == entered_site]
    if entered_site == 'ALL':
